...
class Mitiq_execution:
    ...
    def get_standard_error(self,noise_sim,circuit,num_qubits,configuration):
        ...
        if self.name == "zne":
            ...
            exps = []
            for x in range(10):
                print("sim=",circuit.num_qubits,self.std_obs.num_qubits)
    ...
    def get_mitigation(self,name,circuit,runs,configuration):
        if name=="zne":
            ...
            [cdr.execute_with_cdr(circuit,executor=self.execute_noise,simulator=self.execute_no_noise) for _ in range(runs)]
...
if __name__=="__main__":
    for qubits in range(2,11):
        ...
        zne_configs = Mitiq_execution("zne",[],[]).zne_configurations.keys()
        for zne_conf in zne_configs:
            for index,program_data in enumerate(data):
                #if index>1:
                #    break
                zne_mitiq_executor = Mitiq_execution("zne",program_data["mutant_observable_ideal"],
                                                     program_data["mutant_observable_noisy"])

                std_err = zne_mitiq_executor.get_standard_error(noise_sim,program_data["mutant_circuit_noisy"],qubits,zne_conf)
                mitigated_exps = zne_mitiq_executor.get_mitigation("zne",program_data["mutant_circuit_noisy"],10,zne_conf)
                result = {}
                if os.path.exists(r"./results/{}&zne&{}.json".format(backend_name,zne_conf)):
                    file = open(r"./results/{}&zne&{}.json".format(backend_name,zne_conf),"r")
                    result = json.load(file)
                    file.close()

                file = open(r"./results/{}&zne&{}.json".format(backend_name,zne_conf),"w")
