OPENQASM 2.0;
include "qelib1.inc";
qreg qregless[10];
creg cregless[9];
h qregless[0];
h qregless[1];
h qregless[2];
h qregless[3];
h qregless[4];
h qregless[5];
h qregless[6];
h qregless[7];
h qregless[8];
x qregless[9];
cp(-2.693670263527186) qregless[9],qregless[0];
cp(0.8958447801252144) qregless[9],qregless[1];
cp(1.7916895602504288) qregless[9],qregless[2];
cp(-2.6998061866787286) qregless[9],qregless[3];
cp(0.8835729338221293) qregless[9],qregless[4];
cp(9*pi/16) qregless[9],qregless[5];
cp(-7*pi/8) qregless[9],qregless[6];
cp(pi/4) qregless[9],qregless[7];
cp(pi/2) qregless[9],qregless[8];
swap qregless[0],qregless[8];
h qregless[0];
swap qregless[1],qregless[7];
cp(-pi/2) qregless[1],qregless[0];
h qregless[1];
swap qregless[2],qregless[6];
cp(-pi/4) qregless[2],qregless[0];
cp(-pi/2) qregless[2],qregless[1];
h qregless[2];
swap qregless[3],qregless[5];
cp(-pi/8) qregless[3],qregless[0];
cp(-pi/4) qregless[3],qregless[1];
rxx(3*pi/2) qregless[3],qregless[2];
cp(-pi/2) qregless[3],qregless[2];
h qregless[3];
cp(-pi/16) qregless[4],qregless[0];
cp(-pi/8) qregless[4],qregless[1];
cp(-pi/4) qregless[4],qregless[2];
cp(-pi/2) qregless[4],qregless[3];
h qregless[4];
cp(-pi/32) qregless[5],qregless[0];
cp(-pi/16) qregless[5],qregless[1];
cp(-pi/8) qregless[5],qregless[2];
cp(-pi/4) qregless[5],qregless[3];
cp(-pi/2) qregless[5],qregless[4];
h qregless[5];
cp(-pi/64) qregless[6],qregless[0];
cp(-pi/32) qregless[6],qregless[1];
cp(-pi/16) qregless[6],qregless[2];
cp(-pi/8) qregless[6],qregless[3];
cp(-pi/4) qregless[6],qregless[4];
cp(-pi/2) qregless[6],qregless[5];
h qregless[6];
cp(-pi/128) qregless[7],qregless[0];
cp(-pi/64) qregless[7],qregless[1];
cp(-pi/32) qregless[7],qregless[2];
cp(-pi/16) qregless[7],qregless[3];
cp(-pi/8) qregless[7],qregless[4];
cp(-pi/4) qregless[7],qregless[5];
cp(-pi/2) qregless[7],qregless[6];
h qregless[7];
cp(-pi/256) qregless[8],qregless[0];
cp(-pi/128) qregless[8],qregless[1];
cp(-pi/64) qregless[8],qregless[2];
cp(-pi/32) qregless[8],qregless[3];
cp(-pi/16) qregless[8],qregless[4];
cp(-pi/8) qregless[8],qregless[5];
cp(-pi/4) qregless[8],qregless[6];
cp(-pi/2) qregless[8],qregless[7];
h qregless[8];
barrier qregless[0],qregless[1],qregless[2],qregless[3],qregless[4],qregless[5],qregless[6],qregless[7],qregless[8],qregless[9];
measure qregless[0] -> cregless[0];
measure qregless[1] -> cregless[1];
measure qregless[2] -> cregless[2];
measure qregless[3] -> cregless[3];
measure qregless[4] -> cregless[4];
measure qregless[5] -> cregless[5];
measure qregless[6] -> cregless[6];
measure qregless[7] -> cregless[7];
measure qregless[8] -> cregless[8];
